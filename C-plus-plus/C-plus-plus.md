...
// 没有 volatile 时，编译器可能优化为 while(true)
while (!flag) {
    // 等待 flag 变为 true
}
```

- 没有 `volatile`：编译器可能认为 `flag` 在循环中不会改变，优化为死循环
- 有 `volatile`：编译器每次都会从内存读取 `flag` 的值

2. **用于硬件寄存器访问**

```cpp
// 映射到硬件寄存器的地址
volatile uint32_t* const DEVICE_STATUS = reinterpret_cast<uint32_t*>(0x40021000);

// 轮询设备状态的函数
bool wait_device_ready() {
    // 轮询：不断读取状态寄存器
    while (*DEVICE_STATUS & 0x01) {  // 检查BUSY位
        // volatile确保每次循环都重新从硬件读取
    }
    
    // 检查READY位
    return (*DEVICE_STATUS & 0x02) != 0;
}
```

3. **多线程/中断上下文中`while` 循环持续检查信号/标志位**
...
void interrupt_handler() {
    shared_data = read_sensor();  // 可能在任何时候被调用
}
```

重要注意事项：

`volatile`用于修饰成员或变量，指示其修饰对象可能随时变化，编译器不要对所修饰变量进行优化（缓存），每次取值应该直接读取内存。由于`volatile`的变化来自运行期，其可以与`const`一起使用。两者一起使用可能让人费解，如果考虑场景就容易许多：CPU和GPU通过映射公用内存中的同一块，GPU可能随时往共享内存中写数据。对CPU上的程序来说，`const`修饰变量一直是右值，所以编译通过。但其变量内存中的值在运行期间可能随时在改变，`volatile`修饰是正确做法。

在`volatile`的修饰下，每次循环都会检查内存中的值，达到同步的效果。

需要注意的是，`volatile`的值可能随时会变，期间会导致非预期的结果。例如下面的例子求平方和：
